 ...
 -->							<div class="card noborder bg-white rounded shadow-sm">
								...
								<div class="card-body text-secondary p-4">
									...
									<div class="alert alert-warning" role="alert">
										ویرایش <a href="{link}{shortaddress}{/link}" class="font-weight-bold">لینک</a> در حالت مدیریت
									</div>
									<?php endif; ?>
									...
									{link}
									<?php if($this->session->userdata('admin')): ?>
									<form method="POST" action='<?=base_url("dashboard/admin/links/validate_edit/{id}")?>'>
									<?php endif; ?>
									<form method="POST" action='<?=base_url("dashboard/links/validate_edit/{id}")?>'>
									<?php
									$csrf = array(
...
<div class="input-group mb-4 col-md-6">
    <div class="input-group-prepend">
        <div class="input-group-text"style="min-width: 140px">آدرس اصلی</div>
    </div>


    <input dir="ltr" type="text" value="{address}" name="address" class="form-control <?php echo(array_key_exists('address', $errors))?'is-invalid':''; ?>" placeholder="<?php echo(array_key_exists('address', $errors))?strip_tags(form_error('address')):'آدرس اصلی'; ?>">
...
<span class="badge badge-warning border border-dark font-weight-normal mr-1" style="font-size: 11px;">نکته</span> نام و نام خانوادگی فارسی و کامل صاحب حساب(در صورت ارسال نام به انگلیسی یا بصورت ناقص ، پرداخت با مشکل روبرو خواهد شد)
 ...
 <div class="input-group mb-4 col-md-6">
    <div class="input-group-prepend">
        <div class="input-group-text"style="min-width: 140px">آدرس کوتاه شده</div>
    </div>
...
<div class="d-flex row">
 <div class="input-group mb-4 col-md-6">
    <div class="input-group-prepend">
        <div class="input-group-text"style="min-width: 140px">تاریخ ایجاد</div>
    </div>
 ...
 <div class="input-group mb-4 col-md-6">
    <div class="input-group-prepend">
        <div class="input-group-text"style="min-width: 140px">تاریخ ویرایش</div>
    </div>
 ...
 <div class="row">
	...
	<div class="input-group mb-4 col-md-6">
		...
		<select class="custom-select <?php echo(array_key_exists('type', $errors))?'is-invalid':''; ?>" id="type" name="type">
			<option value="direct" <?=($link[0]['type'] == 'direct') ? 'selected' : '';?>>مستقیم</option>
			<option value="ptc" <?=($link[0]['type'] == 'ptc') ? 'selected' : '';?>>درآمد زا</option>
		</select>
	</div>
</div>
...
<span class="text-danger">
	<?= form_error('type') ?> 
	<?= form_error('button_name') ?>
</span>

<div class="mb-4 d-flex row">
	<div class="col-md-4 mb-2 mt-2 align-self-center">
		<div class="custom-control custom-checkbox">
		  <input type="checkbox" name="private_status" <?php echo($link[0]['private_status'] == 1) ? 'checked' : '';?>  class="custom-control-input" id="password_status">
		  <label class="custom-control-label" for="password_status">برای خصوصی (رمز دار) کردن لینک کلیک کنید</label>
		</div>
	</div>
	<div class="col" id="password" style="display: <?php echo($link[0]['private_status'] == 1) ? 'block' : 'none';?> ;">
